...
export default function FAQSection() {
  return (
    <section
      className="section-light relative overflow-x-clip overflow-y-visible border-y border-black bg-spark"
      aria-labelledby="faq-heading"
    >
      <div
        id="faqs"
        className="relative z-10 scroll-mt-14 md:scroll-mt-16 min-h-[360px] px-3 pb-10 pt-4 md:min-h-[430px] md:px-3 md:pb-12 md:pt-6"
      >
        <h2
          id="faq-heading"
          className="absolute left-0 top-4 font-display font-black tracking-tighter text-ink leading-[0.85] text-7xl sm:text-8xl md:top-6 md:text-[112px] lg:text-[140px]"
        >
          FAQ
        </h2>
        <motion.div
          ...
          className="relative mx-auto grid max-w-[1440px] grid-cols-1 items-start gap-8 md:grid-cols-12 md:gap-10"
        >
          <div className="hidden md:col-span-5 lg:col-span-5 md:block" aria-hidden />

          {/* Right column — collapsed questions with circular arrows */}
          <div className="md:col-span-7 md:col-start-6 lg:col-span-7 md:pt-14 lg:pt-16">
            <Accordion type="single" collapsible className="w-full">
              {FAQS.map((item, i) => (
                ...
                >
                  <AccordionTrigger
                    className="group flex items-center justify-start gap-4 py-3 md:py-3.5 text-left hover:no-underline [&>svg:last-of-type]:hidden"
                  >
                    ...
                    <span className="font-nav text-sm font-medium uppercase leading-[1.15] tracking-[0.02em] text-ink md:text-base">
                      {item.q}
                    </span>
                  </AccordionTrigger>
                  <AccordionContent>
            ...
            <div className="mt-6 flex flex-col items-start gap-3 pl-11 md:mt-7">
              <p className="font-nav text-sm font-medium uppercase leading-none tracking-[0.02em] text-ink/80 md:text-base">
                ¿Todavía tienes preguntas?
              </p>
              <a
                href="/demo"
                className="group relative inline-flex items-center justify-center overflow-hidden rounded-full px-8 py-4 font-nav text-sm font-medium uppercase leading-none tracking-[0.02em] text-white ring-1 ring-white/10 shadow-[0_8px_24px_-10px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.18)] transition-[background-color,box-shadow] duration-300 ease-out hover:ring-white/15 hover:shadow-[0_14px_30px_-12px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.22)] focus:outline-none focus-visible:ring-4 focus-visible:ring-iris/40"
                style={{ backgroundColor: '#7B2CFF' }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.backgroundColor = '#5E1395';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.backgroundColor = '#7B2CFF';
                }}
              >
                <span
                  aria-hidden
