...
export default function Navbar({ showBackLink = false, backHref = '/' }) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 liquid-glass overflow-visible border-b border-canvas/10">
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 flex items-center justify-between h-16">
        <div className="flex items-center gap-4">
          <a href="#" className="font-display font-black text-2xl tracking-tighter text-canvas">
            ANZA
          </a>
          {showBackLink && (
            <a
              href={backHref}
              className="font-mono text-[11px] uppercase tracking-widest text-canvas/60 hover:text-canvas transition-colors"
            >
              ← Volver
            </a>
          )}
        ...
        <div className="hidden md:flex items-center gap-10">
          {links.map(l => (
            <a
              key={l.label}
              href={l.href}
              className="font-mono text-xs uppercase tracking-widest text-canvas/60 hover:text-canvas transition-colors"
            >
              {l.label}
            </a>
          ))}
        ...
        {/* Mobile toggle */}
        <button onClick={() => setOpen(!open)} className="md:hidden p-2 text-canvas">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="md:hidden bg-ink border-b border-canvas/10 overflow-hidden"
          >
            <div className="px-6 py-8 flex flex-col gap-6">
              {links.map(l => (
                <a
                  ...
                  className="font-mono text-sm uppercase tracking-widest text-canvas/60 hover:text-canvas"
                >
                  {l.label}
                </a>
              ))}
