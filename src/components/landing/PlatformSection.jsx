...
function Tile({ feature, delay, applyGridPlacement }) {
  return (
    <motion.article
      ...
      className="flex min-h-[200px] flex-col overflow-visible bg-gradient-to-b from-canvas via-[#FFFCE8]/80 to-spark/[0.18] text-left md:min-h-[240px] lg:min-h-[300px]"
    >
      ...
      <div className="min-w-0 shrink-0 overflow-hidden px-6 pb-0 pt-5">
        <p title={`${feature.title}. ${feature.body}`} className="min-w-0 truncate whitespace-nowrap font-display leading-snug">
          <strong className="font-bold text-ink text-[164px] md:text-[14px] lg:text-[15px]">
            {feature.title}.
          </strong>{' '}
          <span className="font-normal text-ink/55 text-[11px] md:text-[12px] lg:text-[13px]">
            {feature.body}
          </span>
        </p>
      </div>
...
export default function PlatformSection() {
  ...
  return (
    <section
      className="relative z-0 bg-canvas border-b border-ink/10"
    ...
    >
      <div className="max-w-[1440px] mx-auto px-6 md:px-10 pt-12 md:pt-16 lg:pt-20 pb-16 md:pb-24 lg:pb-28">
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-px bg-ink/10 md:grid-cols-6 md:grid-rows-[auto_auto]">
            {FEATURES.map((f, i) => (
