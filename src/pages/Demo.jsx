    ...
    <div className="min-h-screen bg-canvas font-display flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-start pt-28 md:pt-32 pb-16 px-6 md:px-10">
        ...
        {submitted ? (
          ...
          >
            <span className="font-mono text-[10px] uppercase tracking-widest text-ink/50 block mb-4">Confirmado</span>
            <h2 className="font-display font-black text-3xl tracking-tight text-ink">¡Tu demo está reservada!</h2>
            <p className="mt-4 font-display text-ink/60 text-sm leading-relaxed">
              Te hemos enviado un correo de confirmación a <strong>{form.email}</strong> con todos los detalles.
            </p>
            <Link to="/" className="mt-8 inline-block bg-ink text-canvas font-mono text-xs uppercase tracking-widest px-8 py-4 hover:bg-ink/80 transition-colors">
              Volver al inicio
            </Link>
          </motion.div>
        ) : (
          ...
          >
            ...
            <div className="border-b border-ink/10 px-6 py-4 flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className={`font-mono text-[10px] uppercase tracking-widest ${step === 1 ? 'text-ink' : 'text-ink/40'}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`font-mono text-[10px] uppercase tracking-widest ${step === 2 ? 'text-ink' : 'text-ink/40'}`}>
                  Elige tu horario
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row">
              {/* Left panel */}
              <div className="md:w-[45%] p-6 md:p-8 border-b md:border-b-0 md:border-r border-ink/10">
                <h3 className="font-display font-bold text-lg text-ink">Demo de Anza</h3>
                <p className="mt-2 font-display text-sm text-ink/50 leading-relaxed">
                ...
                {step === 1 ? (
                  ...
                  <div className="mt-6 space-y-3">
                    <div className="p-3 bg-canvas border border-ink/10">
                      <span className="font-mono text-[10px] uppercase tracking-widest text-ink/40">Email</span>
                      <p className="font-display text-sm text-ink mt-0.5">{form.email}</p>
                    </div>
                    <div className="p-3 bg-canvas border border-ink/10">
                      <span className="font-mono text-[10px] uppercase tracking-widest text-ink/40">Nombre</span>
                      <p className="font-display text-sm text-ink mt-0.5">{form.name}</p>
                    </div>
                    <button
                      onClick={() => setStep(1)}
                    ...
                    {selectedDay && selectedTime && (
                      <button
                        ...
                        className="w-full mt-4 bg-spark text-ink font-mono text-xs uppercase tracking-widest py-4 flex items-center justify-center gap-2 hover:bg-ink hover:text-canvas transition-colors"
                      >
              ...
              <div className="md:w-[55%] p-6 md:p-8">
                {step === 1 ? (
                  <div className="h-full flex items-center justify-center">
                    <div className="text-center p-6 border border-ink/10 max-w-xs">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-7 mb-2">
                      {WEEK_DAYS.map(d => (
                        <div key={d} className="text-center font-mono text-[10px] uppercase tracking-widest text-ink/30 py-1">
                          {d}
                        </div>
                      ))}
                    </div>
                        ...
                        return (
                          <button
                            ...
                            className={`
                              ...
                              ${isPast ? 'text-ink/20 cursor-not-allowed' : 'hover:bg-spark cursor-pointer'}
                              ${isSelected ? 'bg-ink text-canvas' : ''}
                              ${isTodayDay && !isSelected ? 'border border-ink' : ''}
                            `}
                          >
                            {format(day, 'd')}
                          </button>
                          ...
                          {TIMES.map(t => (
                            <button
                              ...
                              className={`
                                ...
                                ${selectedTime === t ? 'bg-ink text-canvas border-ink' : 'border-ink/20 text-ink hover:border-ink hover:bg-spark'}
                              `}
                            >
                              {t}
                            </button>
            ...
            {/* Footer */}
            <div className="border-t border-ink/10 px-6 py-3 text-center">
              <span className="font-mono text-[10px] text-ink/30 uppercase tracking-widest">Powered by Anza</span>
            </div>
          </motion.div>
        )}
